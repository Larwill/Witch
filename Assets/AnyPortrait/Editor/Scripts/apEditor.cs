	...
	public class apEditor : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			try
			{
				...
				//Left, Right UI를 숨기는 좌, 우 Fold 버튼
				GUIStyle guiStyle_FoldBtn = new GUIStyle(GUI.skin.label);
				guiStyle_FoldBtn.margin = GUI.skin.button.margin;
				guiStyle_FoldBtn.alignment = TextAnchor.MiddleCenter;
		...
		private void GUI_Top(int width, int height)
		{
			...
			SetGUIVisible("Top UI - Position", (gizmoUI_Position2D != apGizmos.TRANSFORM_UI_VALID.Hide));
			...
			SetGUIVisible("Top UI - Depth", (gizmoUI_Depth != apGizmos.TRANSFORM_UI_VALID.Hide));
			SetGUIVisible("Top UI - Color", (gizmoUI_Color != apGizmos.TRANSFORM_UI_VALID.Hide));
			SetGUIVisible("Top UI - Extra", (gizmoUI_Extra != apGizmos.TRANSFORM_UI_VALID.Hide));

			SetGUIVisible("Top UI - BoneIKController", (gizmoUI_BoneIKController != apGizmos.TRANSFORM_UI_VALID.Hide));
			...
			SetGUIVisible("Top UI - VTF Blur", (gizmoUI_VetexTF != apGizmos.TRANSFORM_UI_VALID.Hide) && Gizmos.IsBrushMode);
			...
			isGizmoGUIVisible_Position2D = IsDelayedGUIVisible("Top UI - Position");
			isGizmoGUIVisible_Rotation = IsDelayedGUIVisible("Top UI - Rotation");
			isGizmoGUIVisible_Scale = IsDelayedGUIVisible("Top UI - Scale");
			isGizmoGUIVisible_Depth = IsDelayedGUIVisible("Top UI - Depth");
			isGizmoGUIVisible_Color = IsDelayedGUIVisible("Top UI - Color");
			isGizmoGUIVisible_Extra = IsDelayedGUIVisible("Top UI - Extra");
				...
				|| isGizmoGUIVisible_BoneIKController)
			{
				...
				if (isGUITab_GizmoValue)
				{
					...
					if (isGizmoGUIVisible_Depth)
					{
						...
						EditorGUILayout.LabelField(GetUIWord(UIWORD.Depth), guiStyle_Label, GUILayout.Width(70));//"Depth"
						if (gizmoUI_Depth == apGizmos.TRANSFORM_UI_VALID.ShowAndEnabled)
						{
					...
					if (isGizmoGUIVisible_Color)
					{
						...
						EditorGUILayout.BeginVertical(GUILayout.Width(70));
						EditorGUILayout.LabelField(GetUIWord(UIWORD.Color), guiStyle_Label, GUILayout.Width(70));//"Color"
						if (gizmoUI_Color == apGizmos.TRANSFORM_UI_VALID.ShowAndEnabled)
						{
						...
						EditorGUILayout.LabelField(GetUIWord(UIWORD.Visible), guiStyle_Label, GUILayout.Width(45));//"Visible"
						if (gizmoUI_Color == apGizmos.TRANSFORM_UI_VALID.ShowAndEnabled)
						{
					...
					//추가 5.9 : Bone의 IK Controller의 Mix Weight를 설정해야한다.
					//if(isGizmoGUIVisible_BoneIKController)
					//{
					//	GUIStyle guiStyle_Label = new GUIStyle(GUI.skin.label);
					//	if (gizmoUI_BoneIKController != apGizmos.TRANSFORM_UI_VALID.ShowAndEnabled)
					//	{
					//		guiStyle_Label.normal.textColor = Color.gray;
					//	}

					//	EditorGUILayout.LabelField(new GUIContent(ImageSet.Get(apImageSet.PRESET.Transform_IKController)), GUILayout.Width(30), GUILayout.Height(30));
					//	EditorGUILayout.BeginVertical(GUILayout.Width(120));
					//	EditorGUILayout.LabelField(GetUIWord(UIWORD.IKConModWeight), guiStyle_Label, GUILayout.Width(120));//"FK/IK Weight"<언어
					//	if (gizmoUI_BoneIKController == apGizmos.TRANSFORM_UI_VALID.ShowAndEnabled)
					//	{
					//		curBoneIKMixWeight = EditorGUILayout.DelayedFloatField("", prevBoneIKMixWeight, GUILayout.Width(60));
					//	}
					//	else
					//	{
					//		EditorGUILayout.DelayedFloatField("", 0, GUILayout.Width(60));
					//	}
					//	EditorGUILayout.EndVertical();
		...
		private void GUI_MainRight_Lower_AnimationHeader(int width, int height)
		{
			...
			//삭제 19.8.18 : Layout 숨기는 버튼이 상단으로 이동됨
